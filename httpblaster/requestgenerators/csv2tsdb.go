...
func (c *Csv2TSDB) generateRequest(chRecords chan []string, chReq chan *Request, host string,
	wg *sync.WaitGroup) {
	defer wg.Done()
...
func (c *Csv2TSDB) generate(chReq chan *Request, payload string, host string) {
	defer close(chReq)
